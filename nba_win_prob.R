...
# Read in tsv files for PbP data
pbp2012 <- read.table("~/Downloads/AllData20120510040/playbyplay20120510040.txt", sep="\t", header=TRUE)
pbp2011 <- read.table("~/Downloads/playbyplay20102011reg20110416.txt", sep="\t", header=TRUE)
pbp2010 <- read.table("~/Downloads/playbyplay20092010reg20100418.txt", sep="\t", header=TRUE)
pbp2009 <- read.table("~/Downloads/playbyplay20082009reg20090420.txt", sep="\t", header=TRUE)
pbp2008 <- read.table("~/Downloads/playbyplay20072008reg20081211.txt", sep="\t", header=TRUE)
pbp2007 <- read.table("~/Downloads/playbyplay200704201041.txt", sep="\t", header=TRUE)
...
odds$v <- str_replace_all(odds$v, "New Orleans Pelicans", "NOR") %>%
...
odds$h <- str_replace_all(odds$h, "New Orleans Pelicans", "NOR") %>%
    str_replace_all("San Antonio Spurs", "SAS") %>%
    str_replace_all("Milwaukee Bucks", "MIL") %>%
    str_replace_all("Denver Nuggets", "DEN") %>%
    str_replace_all("Phoenix Suns", "PHX") %>%
...
pbp <- ungroup(pbp) %>% select(date, v, h, lineV, timeRemainMin, ptDiffV, possV, winV)
